...
const EditOption: React.FC<Props> = ({className, title, content, onSubmit, value, onChange}) => {
    ...
    return (
        <>
            <div className={`flex justify-between ${className} ${isActive ? "hidden" : "block"}`}>
                <div>
            ...
            <div className={`${className} ${isActive ? "block" : "hidden"}`}>
                <form className="text-sm" onSubmit={onSubmit}>
                    ...
                    <div className="flex items-center justify-between">
                        <input
                            className="flex-grow p-2 mr-0.5 rounded outline-none border border-[#e6ebf5] bg-[#f9fafc] text-black dark:border-[#303841] dark:bg-[#5e656e] dark:text-[#81a0cf]"
                            type="text"
